...
const Cancel = () => {
  ...
  const handleCancelRoute = async () => {
    if (selectedRoute) {
      try {
        const res = await axios.post('http://localhost:4002/customer/cancel', { id: selectedRoute.id, uname: uname }, {
          headers: {},
          withCredentials: true,
        });

        if (res) {
          toast.success('Route cancelled successfully!', { position: 'top-right', autoClose: 2500 });
          setIsModalOpen(false);  // Close the modal
        }
      } catch (error) {
        toast.error('Cancelation failed!', { position: 'top-right', autoClose: 2500 });
      }
    }
  };
      ...
      {isModalOpen && (
        <div style={styles.modal}>
          <div style={styles.modalContent}>
            <h2>Do you want to Cancel this Route?</h2>
            <div>
              <button onClick={handleCancelRoute} style={styles.button}>Yes</button>
